...
template <FramebufferDims FBDIMS>
bool MsxPPU<FBDIMS>::loadIndexedImagePNG(const uint8_t* pData, size_t data_size, uint32_t vram_address, uint16_t img_width, uint16_t img_height, Palette<16>* pPalette) {
	std::vector<uint8_t> img;
	if(!Utils::loadIndexedPng<16>(pData, data_size, img_width, img_height, img, pPalette)) {
		return false;
	}

	// TODO: check VRAM boundaries
	for(size_t i = 0; i < img_height; ++i) {
		const uint8_t* pSrc = img.data() + i * img_width;
...
bool MsxPPU<FBDIMS>::loadIndexedImagePNG(const std::string& filename, uint32_t vram_address, uint16_t img_width, uint16_t img_height, Palette<16>* pPalette) {
	std::vector<uint8_t> img;
	if(!Utils::loadIndexedPng<16>(filename, img_width, img_height, img, pPalette)) {
		return false;
	}
