...
uniform uint    uFrontendFrameCount;
...
vec3 Tri(vec2 uv) {
    ...
    float line = floor(warp_uv.y * uVideoResolution.y);
    float tex_u = ((warp_uv.x - 0.5) / uPicStretch.x) + 0.5;

    vec3 sampleA = sampleScanlineLinear(uVideoTexture, tex_u, int(line-1), uVideoResolution).rgb;
    vec3 sampleB = sampleScanlineLinear(uVideoTexture, tex_u, int(line), uVideoResolution).rgb;
    vec3 sampleC = sampleScanlineLinear(uVideoTexture, tex_u, int(line+1), uVideoResolution).rgb;

    vec4 osdSampleA = sampleScanlineLinear(uOSDTexture, tex_u, int(line-1), uOSDResolution);
    vec4 osdSampleB = sampleScanlineLinear(uOSDTexture, tex_u, int(line), uOSDResolution);
    vec4 osdSampleC = sampleScanlineLinear(uOSDTexture, tex_u, int(line+1), uOSDResolution);

    vec3 osdOverdrive = vec3(1.5, 1.5, 1.5);
    sampleA = mix(sampleA.rgb, osdSampleA.rgb * osdOverdrive, osdSampleA.a);
    sampleB = mix(sampleB.rgb, osdSampleB.rgb * osdOverdrive, osdSampleB.a);
    sampleC = mix(sampleC.rgb, osdSampleC.rgb * osdOverdrive, osdSampleC.a);
    ...
    float scan_off = uScanFlicker * (int(uFrontendFrameCount) % 2) / 2.0;
...
void main() {
    ...
    float edgeDefocus = uEdgeDefocus;
    float aberration = uAberration * 0.01;
    vec2 misalignment = uMisalignment * 0.001; //vec2(0.0005, -0.0005);//vec2(-0.00085, 0.00025);
    ...
    if(edgeDefocus > 0.00001 || aberration > 0.00001) {
        ...
        color = color * 0.2 + vec3(colorR, colorG, colorB) * 0.8;
    }
