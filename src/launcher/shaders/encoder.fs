...
void main() {
    ...
    for(int i=-3; i<=3; i++) {
        ...
        vec3 yc = rgb2ycbccr(sampleScanlineLinear(uInputTex, uv.x, line, vec2(uInW, uInH)).rgb);
    ...
    if(uConnType == 3u) {
        ...
        // COMPONENT mode
        fragColor = vec4(rgb2ycbccr(sampleScanlineLinear(uInputTex, vTexCoord.x, line, vec2(uInW, uInH)).rgb), 1.0);
    } else {
